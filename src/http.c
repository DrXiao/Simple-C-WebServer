...
struct http_response_msg_t {
    int client_sockfd;
    struct http_status_table_t *status;
...
static struct http_status_table_t http_status_table[] = {
    {HTTP_OK, "OK"},
    {HTTP_NOT_MODIFIED, "Not Modified"},
    {HTTP_NOT_FOUND, "Not Found"},
};

static struct mime_type_t mime_type[] = {{".html", "text/html"},
...
static void http_parse(struct http_request_msg_t *http_req_msg,
    ...
    // TODO: Parse request line, header and body
...
static void http_parse_request_line(struct http_request_msg_t *http_req_msg,
    ...
    if (!strncmp(uri, "/", (size_t)(http_ver - uri) - 1))
        strcat(http_res_msg->filename, "homepage.html");

    int mime_idx = 0;
    for (; mime_idx < sizeof(mime_type) / sizeof(struct mime_type_t) - 1;
         mime_idx++) {
...
static void
http_parse_request_header(struct http_request_msg_t *http_req_msg,
                          struct http_response_msg_t *http_res_msg) {
    // printf("%s", http_req_msg->header);
}
...
static void http_reply(struct http_response_msg_t *http_res_msg) {
    ...
    snprintf(http_res_msg->header, sizeof(http_res_msg->header),
             ...
             http_res_msg->status->status_name, "CServer",
             http_res_msg->mime->value, http_res_msg->file_stat.st_size);
...
static void http_reply_err(struct http_response_msg_t *http_res_msg) {
    ...
    snprintf(http_res_msg->header, sizeof(http_res_msg->header),
             ...
             HTTP_VER, http_res_msg->status->status_code,
             http_res_msg->status->status_name, "CServer",
             http_res_msg->mime->value, strlen(http_res_msg->body));
    send(http_res_msg->client_sockfd, http_res_msg->header,
         strlen(http_res_msg->header), MSG_NOSIGNAL);
